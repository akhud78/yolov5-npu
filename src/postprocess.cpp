...
// Define class labels for object detection
...
// Anchor boxes for different strides
...
// Quick sort indices in descending order
...
// Quantize float to int8 using affine transformation
static int8_t qnt_f32_to_affine(float f32, int32_t zp, float scale) {
    float dst_val = (f32 / scale) + zp;
    return static_cast<int8_t>(__clip(dst_val, -128, 127));
}

// Dequantize int8 to float using affine transformation
static float deqnt_affine_to_f32(int8_t qnt, int32_t zp, float scale) {
    return ((float)qnt - (float)zp) * scale;
}

// Process a single feature map output
...
int post_process(int8_t *input0, int8_t *input1, int8_t *input2,
    ...
    // Containers to store processed results
    std::vector<float> filterBoxes;
    std::vector<float> objProbs;
    std::vector<int> classId;

    // Process feature map with stride 8
    int stride0 = 8;
    ...
    // Process feature map with stride 16
    int stride1 = 16;
    ...
    // Process feature map with stride 32
    int stride2 = 32;
    ...
    // Format the final detection results
    for (int i = 0; i < validCount; ++i) {
